...
if __name__ == "__main__":
    con = connect("entries/Joylog.db")
    cur = con.cursor()

    #Create Tables
    cur.execute("CREATE TABLE joylog(datetime, text, imageID, locationID, score)")
    cur.execute("CREATE TABLE images(imageID, image, filename)")
    cur.execute("CREATE TABLE locations(locationID, long, lat)")

    #Create Indexes
    cur.execute("CREATE INDEX idx_joylog ON joylog(datetime)")
